...
\[
\begin{split}
z &\sim \mathcal{N}(\mu_1, \Sigma_{11}), \hspace{5mm} \Sigma_{11} = \sigma^2 \Omega = \sigma^2\exp\left(-\frac{d_{ij}^2}{2\phi^2}\right) \\
z^* | z &\sim \mathcal{N}(\mu_{2|1}, \Sigma_{2|1})
\end{split}
\]
where $d_{ij}$ represents the distance between two sample locations and $z^*$ represents the predicted spatial random effects at unobserved locations. We assume that
\[
\begin{split}
\begin{bmatrix} z \\ z^* \end{bmatrix} \sim \mathcal{N} \left( \begin{bmatrix} \mu_1 \\ \mu_2 \end{bmatrix}, \begin{bmatrix} \Sigma_{11} & \Sigma_{12} \\ \Sigma_{21} & \Sigma_{22} \end{bmatrix} \right)
\end{split}
\]
and that therefore $\mu_{2|1} = \mu_2 + \Sigma_{21}\Sigma_{11}^{-1}(z - \mu_1) = (\Sigma_{11}^{-1}\Sigma_{12})'(z - \mu_1)$ (assuming a prior mean of 0) and $\Sigma_{2|1} = \Sigma_{22} - (\Sigma_{11}^{-1}\Sigma_{12})'\Sigma_{12}$. 
...
# Non-normal Gaussian process

# Latent Gaussian process with prediction - non-normal spatial process
...
\[
\begin{split}
z_i &\sim \text{Bernoulli}\left(\Phi(\eta_i) \right)\\
\eta &\sim \mathcal{N}(\mu_1, \Sigma_{11}), \hspace{5mm} \Sigma_{11} = \sigma^2 \Omega = \sigma^2\exp\left(-\frac{d_{ij}^2}{2\phi^2}\right) \\
\eta^* | \eta &\sim \mathcal{N}(\mu_{2|1}, \Sigma_{2|1})
\end{split}
\]
where $d_{ij}$ represents the distance between two sample locations and $\eta^*$ represents the predicted spatial random effects at unobserved locations. We assume that
\[
\begin{split}
\begin{bmatrix} \eta \\ \eta^* \end{bmatrix} \sim \mathcal{N} \left( \begin{bmatrix} \mu_1 \\ \mu_2 \end{bmatrix}, \begin{bmatrix} \Sigma_{11} & \Sigma_{12} \\ \Sigma_{21} & \Sigma_{22} \end{bmatrix} \right)
\end{split}
\]
and that therefore $\mu_{2|1} = \mu_2 + \Sigma_{21}\Sigma_{11}^{-1}(\eta_ - \mu_1) = (\Sigma_{11}^{-1}\Sigma_{12})'(\eta - \mu_1)$ (assuming a prior mean of 0) and $\Sigma_{2|1} = \Sigma_{22} - (\Sigma_{11}^{-1}\Sigma_{12})'\Sigma_{12}$. 
...
Data generating values: $\beta = 0$, $\sigma = 1$, $\phi = 3$, $\alpha = 1$.
...
sim_latentgp_predict <- function(n = 100, seed = 1, sigma = 1, phi = 1, mu = 0, X = matrix(1, nrow = n), beta = 0, alpha = 1, delta = 1e-9, obs_y_prop = .6, obs_z_prop = .5, colocate = "none", sp_type = "normal", link = "identity"){
  # useful functions
  distance <- function(x){
    dist.vec <- parallelDist::parDist(x)
    dist <- as.matrix(dist.vec)
  ...
  if(sp_type == "normal" & link == "identity"){
    grid2 <- grid2 %>%
      mutate(
        obs_y = ifelse(obs_y_ind == 1, y, NA),
        obs_z = ifelse(obs_z_ind == 1, z, NA)
      )
  } else if(sp_type == "binomial" & link == "probit"){
    grid2 <- grid2 %>%
      mutate(
        z_bin = rbinom(n(), 1, pnorm(z))
      ) %>%
      mutate(
        obs_y = ifelse(obs_y_ind == 1, y, NA),
        obs_z = ifelse(obs_z_ind == 1, z_bin, NA)
      )
  } else{
    stop("Combination of sp_type and link not available.")
  }
...
p1 <- data$df %>%
  st_as_sf %>%
  mutate(z_bin = factor(z_bin)) %>%
  ggplot() +
  geom_sf(aes(fill = z_bin)) +
  theme_bw() +
  labs(title = "Population spatial effects")
...
We obtain Gibbs draws for the spatial random effects by leveraging the data augmentation strategy of @albert1993 (nice tutorial [here](https://rstudio-pubs-static.s3.amazonaws.com/208180_b659633007eb45aa9c48e4c50b8afc07.html)).

## Gibbs sampler - currently incorrect
...
# sd = 2.4^2
# epsilon = 1e-6
latentgp_mhgibbs_predict_nonnormalsp <- function(
    seed, num_mcmc, warmup = num_mcmc/2, 
    data_list, X_,
    phi_a = .1, phi_b = .1, sigma_a = .1, sigma_b = .1, delta = .0001, 
    initial_prop_var = .01, adapt = TRUE, adapt_period = .1*num_mcmc, sd = 2.4^2, epsilon = 1e-6){
  ...
  XtOX_obs <- t(X_obs) %*% Omega_inv %*% X_obs
  z_probit <- rep(0, n)
  z_probit[z == 0] <- truncnorm::rtruncnorm(
    sum(z == 0), mean = rep(mu, n)[z == 0], sd = 1, a = -Inf, b = 0
  )
  z_probit[z == 1] <- truncnorm::rtruncnorm(
    sum(z == 1), mean = rep(mu, n)[z == 1], sd = 1, a = 0, b = Inf
  )
  ...
  pb <- txtProgressBar(min = 0, max = num_mcmc, style = 3, width = 50, char = "=")
  for(iter in 2:num_mcmc){
    # albert and chibb data augmentation first
    z_probit[z == 0] <- truncnorm::rtruncnorm(
      sum(z == 0), mean = rep(mu, n)[z == 0], sd = 1, a = -Inf, b = 0
    )
    z_probit[z == 1] <- truncnorm::rtruncnorm(
    ...
    XtOz_obs <- t(X_obs) %*% Omega_inv %*% z_probit
    ...
    # sigma
    a <- sigma_a + n/2
    b <- c(sigma_b + .5 * t(z_probit - mu) %*% Omega_inv %*% (z_probit - mu))
    sigma <- sqrt(invgamma::rinvgamma(1, a, b))
    sigma2 <- sigma^2
    ...
    ## evaluate proposal
    log_post_current <- mvtnorm::dmvnorm(z_probit, rep(mu,n), sigma2 * Omega, log = T) + invgamma::dinvgamma(phi, phi_a, phi_b, log = T)
    log_post_s <- mvtnorm::dmvnorm(z_probit, rep(mu,n), sigma2 * Omega_s, log = T) + invgamma::dinvgamma(phi_s, phi_a, phi_b, log = T)
    log_r <- log_post_s - log_post_current
    if(log(runif(1)) < log_r){
      phi <- phi_s
      phi2 <- phi_s^2
    ...
    Sigma2.1 <- Sigma22 - t.Sigma11_inv.Sigma12 %*% Sigma12
    Sigma2.1[lower.tri(Sigma2.1)] = t(Sigma2.1)[lower.tri(Sigma2.1)]
    mu2.1 <- t.Sigma11_inv.Sigma12 %*% (matrix(c(z_probit) - c(mu), ncol = 1))
    eta <- mvtnorm::rmvnorm(1, mu2.1, Sigma2.1 + diag(delta, dim(Sigma2.1)))

    # betas
    pred <- data_list$z_atobsy
    pred[which(!is.na(pred))] <- c(z_probit)
    pred[which(is.na(pred))] <- c(eta)
    model_mat <- cbind(
      X_, 
      pred
...
samples <- parLapply(
  cl = this_cluster,
  X = 1:3,
  fun = latentgp_mhgibbs_predict_nonnormalsp,
  num_mcmc = 10000,
  warmup = 5000,
  data_list = data_list,
  X_ = cbind(
...
stopCluster(this_cluster)
saveRDS(samples, "rds files/latentgp_predict_nonnormalsp/latentgp_predict_nonnormalsp_onefit.rds")
```

```{r, echo = T}
samples <- readRDS("rds files/latentgp_predict_nonnormalsp/latentgp_predict_nonnormalsp_onefit.rds")
...
```{r, eval = F}
# from Emily Almberg email 2022/04/15
pd <- readr::read_csv("data/PdSamplingLocs_AllTime_NABatGRIT_Stratton_4_15_2022.csv")
saveRDS(pd, "data/pd.rds")

# from Dan Bachen email 2022/02/02
activity <- readr::read_csv("data/WNSBats_NABatData.csv")
saveRDS(activity, "data/activity.rds")
```

```{r}
activity <- readRDS("data/activity.rds")
montana <- read_sf("data/Montana")
```
...
# grab site and cell
cell581s4 <- activity %>%
  filter(grepl("Cell 581 Site 4", Location))

# add time
cell581s4 <- cell581s4 %>%
  mutate(
    date = ymd(stringr::str_sub(Filename, start = 8, end = 15)),
    time = stringr::str_sub(Filename, start = 17, end = 22)
  ) %>%
  mutate(
    hour = stringr::str_sub(time, start = 1, end = 2),
    minute = stringr::str_sub(time, start = 3, end = 4),
    second = stringr::str_sub(time, start = 5, end = 6)
  ) %>%
  arrange(
    Filename
  )
...
tmp <- cell581s4 %>% filter(year(date) == 2021) %>% select(Filename,Master_Class, date, time:second)
```

## Pd surveillance data

As of `r Sys.Date()`, none of the Pd surveillance data from Montana caves are included; we are waiting on Dan Bachen to provide "fuzzed" cave locations. 

```{r, eval = F}
pd <- readRDS("data/pd.rds") %>%
  arrange(Site_Summary_ID) %>%
  dplyr::select(
    site_id = Site_Summary_ID,
    county = County,
    wns_summary = `Site_Summary_WNS Case Definition`,
    lat = Lat_case,
    lon = Long_case,
    grts = GRTS_ID,
    date = FinalDate,
...
pd_sf <- pd %>%
  # lose caves for now :()
  filter(!is.na(lon)) %>%
  filter(wns_summary != "NT") %>%
  mutate(
    year = year(mdy(date))
  ) %>%
  mutate(
    wns_resp = factor(case_when(
      wns_summary %in% c("Neg", "Pd not detected") ~ "No Pd",
      wns_summary == "Equiv" ~ "equivocal",
      wns_summary == "Suspect" ~ "suspect",
      TRUE ~ "Pd"
    ))
  ) %>%
...
ggplot() +
  geom_sf(data = montana %>% as_tibble, aes(geometry = geometry)) +
  geom_sf(
    data = pd_sf %>% filter(year %in% c(2020:2021)) %>% as_tibble, 
    aes(geometry = geometry),
    aes(col = wns_resp), size = 2
  ) +
  theme_bw() +
  labs(
    title = "Pd surveillance"
  ) +
  facet_wrap(~ year, nrow = 1)
ggsave("pd_surv.png", width = 20, height = 15, units = "in")
```
