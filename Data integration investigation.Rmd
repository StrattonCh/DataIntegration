...
saveRDS(ols_sims, "ols_sims.rds")
```

```{r}
ols_sims <- readRDS("ols_sims.rds")
ols_sims %>%
  mutate(
...
saveRDS(gls_sims, "gls_sims.rds")
```

```{r}
gls_sims <- readRDS("gls_sims.rds")
gls_sims %>%
  mutate(
    capture = factor(case_when(
      truth >= lwr & truth <= upr ~ 1, 
...
saveRDS(gp_sims, "gp_sims_1_3.rds")
```

```{r}
gp_sims <- readRDS("gp_sims_1_3.rds")
gp_sims %>%
  mutate(
    capture = factor(case_when(
      truth >= lwr & truth <= upr ~ 1, 
...
saveRDS(occ_fit, "fit_msocc_counts_test.rds")
...
saveRDS(msocc_simulation_counts_test, "msocc_simulation_counts_test3.rds")
```

```{r, eval = F}
msocc_simulation_counts_test <- readRDS("msocc_simulation_counts_test.rds")
...
saveRDS(fit, file = "msocc_count_fit_nimble.rds")
...
saveRDS(msocc_simulation_counts, "msocc_simulation_counts.rds")
```

```{r, eval = T}
msocc_simulation_counts <- readRDS("msocc_simulation_counts.rds")
